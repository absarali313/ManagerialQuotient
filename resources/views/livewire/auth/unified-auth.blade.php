<div class="bg-white lg:rounded-2xl lg:shadow-[0_8px_30px_rgb(0,0,0,0.04)] lg:border lg:border-gray-100 p-8 lg:p-10">
    @if(session('error'))
        <div class="mb-6 p-4 rounded-lg bg-red-50 text-red-700 text-sm border border-red-200">{{ session('error') }}</div>
    @endif

    @if(session('status'))
        <div class="mb-6 p-4 rounded-lg bg-green-50 text-green-700 text-sm border border-green-200">{{ session('status') }}</div>
    @endif
    ...
    @if($mode === 'login')
        <form wire:submit="login" class="space-y-6">
            <x-form.input label="Email Address" id="email" type="email" model="email" placeholder="Enter your work email" required>
                <x-slot name="icon">
                    <x-lucide-mail class="w-5 h-5" />
                </x-slot>
            </x-form.input>

            <x-form.input label="Password" id="password" type="password" model="password" placeholder="Enter your password" required>
                <x-slot name="icon">
            ...
            <div class="flex items-center justify-between">
                <x-form.checkbox label="Remember me" id="remember_me" model="isRemembered" />

                <div class="text-sm">
                    <button type="button" wire:click="switchMode('forgot')" class="font-medium text-blue-600 hover:text-blue-500 transition">Forgot password?</button>
                </div>
            </div>
    ...
    @elseif($mode === 'register')
        <form wire:submit="register" class="space-y-5">
            <x-form.input label="Organization Name" id="orgName" model="orgName" placeholder="Acme Corp" required>
                <x-slot name="icon">
                    <x-lucide-building class="w-5 h-5" />
                </x-slot>
            </x-form.input>

            <x-form.input label="Your Full Name" id="name" model="name" placeholder="John Doe" required>
                <x-slot name="icon">
                    <x-lucide-user class="w-5 h-5" />
                </x-slot>
            </x-form.input>

            <x-form.input label="Work Email" id="reg_email" type="email" model="email" placeholder="john@acmecorp.com" required>
                <x-slot name="icon">
                    <x-lucide-mail class="w-5 h-5" />
                </x-slot>
            </x-form.input>

            <x-form.input label="Password" id="reg_password" type="password" model="password" placeholder="Create a strong password" required>
                <x-slot name="icon">
                    <x-lucide-lock class="w-5 h-5" />
                </x-slot>
            </x-form.input>
    ...
    @elseif($mode === 'forgot')
        <form wire:submit="sendPasswordReset" class="space-y-6">
            <x-form.input label="Email Address" id="forgot_email" type="email" model="email" placeholder="Enter your work email" required>
                <x-slot name="icon">
                    <x-lucide-mail class="w-5 h-5" />
                </x-slot>
            </x-form.input>
            ...
            <div class="flex space-x-3">
                <button type="button" wire:click="switchMode('login')" class="w-1/3 flex justify-center py-2.5 px-4 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-150">Back</button>
        ...
        <div class="mt-6">
            <div class="relative">
                ...
                <div class="relative flex justify-center text-sm">
                    <span class="px-3 bg-white text-gray-500 text-xs uppercase tracking-wide">Or continue with</span>
                </div>
            </div>

            <div class="mt-6">
                <a href="{{ route('auth_google') }}" class="w-full inline-flex justify-center items-center py-2.5 px-4 border border-gray-200 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 transition duration-150">
