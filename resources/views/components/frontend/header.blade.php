            ...
            <div class="flex items-center gap-10">
                ...
                <nav class="hidden md:flex items-center gap-6">
                    <a href="{{ route('home') }}" @class([
                        ...
                        'text-blue-600 bg-blue-50' => request()->routeIs('home'),
                        'text-gray-500 hover:text-gray-900' => !request()->routeIs('home')
                    ])>Home</a>
            ...
            <div class="flex items-center gap-4">
                @auth
                    <a href="{{ url('/dashboard') }}"
                       class="inline-flex items-center px-5 py-2.5 text-sm font-bold text-gray-700 hover:text-blue-600 transition-colors bg-gray-50 hover:bg-blue-50 rounded-2xl">
                        Dashboard
                    </a>
                @else
                    <a href="{{ route('login') }}"
                       class="hidden sm:inline-flex text-gray-600 hover:text-gray-900 text-sm font-bold transition-colors">
                        Sign In
                    </a>

                    @if (Route::has('register'))
                        <a href="{{ route('register') }}"
                           class="inline-flex items-center px-6 py-3 text-sm font-bold rounded-2xl shadow-xl shadow-blue-500/25 text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all hover:-translate-y-0.5 active:translate-y-0">
                            Get Started
                        </a>
                    @endif
                @endauth
